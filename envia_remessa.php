...
require_once 'consulta_token.php';
...
class EnviaRemessa {
	...
	public function addBoleto($data = null) {
		if(!empty($data)) {
			...
			//setando o juros
			$juros = new Juros();
			$juros->setTipoJuros($data['juros']['tipo_juros']);
			$juros->setPercentualJuros($data['juros']['percentual_juros']);
			
			$boleto->setJuros($juros);

			//setando a multa
			$multa = new Multa();
			$multa->setTipoMulta($data['multa']['tipo_multa']);
			$multa->setPercentualMulta($data['multa']['percentual_multa']);
